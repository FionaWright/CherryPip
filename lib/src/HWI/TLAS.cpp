...
void TLAS::Init(ID3D12Device5* device, ID3D12GraphicsCommandList4* cmdList, const std::vector<std::shared_ptr<BLAS>>& blasList)
{
    ...
    m_tlasScratch.InitBuffer(L"TLAS Scratch", device, prebuildInfo.ScratchDataSizeInBytes, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    m_tlasResult.InitRTAS(L"TLAS Result", device, prebuildInfo.ResultDataMaxSizeInBytes, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
