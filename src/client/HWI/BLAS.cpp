...
void BLAS::Init(ID3D12Device5* device, ID3D12GraphicsCommandList4* cmdList, const std::shared_ptr<Model>& model, const Transform& transform)
{
    ...
    m_blasScratch.InitBuffer(L"BLAS Scratch", device, prebuildInfo.ScratchDataSizeInBytes, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    m_blasResult.InitRTAS(L"BLAS Result", device, prebuildInfo.ResultDataMaxSizeInBytes, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
