...
void EnvMap::InitCubemap(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList, Heap* heap)
{
    ...
    m_matPanoToCM.SetSRV(device, 0, heap, m_pano.GetD12Resource(), srvDesc);
    ...
    uint32_t groupSizeXY = (m_cubemap.GetDesc().Width + 15) / 16;
